...
%DIF DEL StableMatchingPolytope.tex            Thu May 12 16:17:26 2016
%DIF ADD StableMatchingPolytope-Revision.tex   Thu Jul 14 09:34:49 2016
...
\end{lemma}
\begin{proof}
  Since $M_1$ and $M_2$ are matchings, $C$ is a path or a cycle \DIFaddbegin \DIFadd{with edges alternating between $M_1$ and $M_2$}\DIFaddend . Let
  ...
  W.l.o.g. \DIFdelbegin \DIFdel{$v:=a\in \mathcal{A}$ }\DIFdelend \DIFaddbegin \DIFadd{$a:=v,\ a\in \mathcal{A}$ }\DIFaddend and $b:=M_1(a) >_a M_2(a)$. If
...
\end{tikzpicture}
\caption{Visualizing the proof of Lemma \ref{lemma:pref}. Here, the connected component $C$ is a cycle on four nodes. The edges of $M_1$ are marked by straight blue lines, and the edges of $M_2$ by red zigzags. \DIFaddbeginFL \DIFaddFL{So here $a_1 = M_2(b)$, $b_1 = M_1(M_2(b))$. }\DIFaddendFL For each node $w\in \{a,b,a_1,b_1\}$, the arrow at $w$ points towards the most prefered node in $\{M_1(w), M_2(w)\}$ with respect to $>_w$. }
...
\node[shape=circle,draw=black,inner sep=0pt, minimum size=4pt, label=right:$b_1$](b1) at (2,4){};
\node[shape=circle,draw=black,inner sep=0pt, minimum size=4pt, label=left:$a_2$] (a2) at (0,2) {};
\node[shape=circle,draw=black,inner sep=0pt, minimum size=4pt, label=right:$b$](b2) at (2,2){};
\node[shape=circle,draw=black,inner sep=0pt, minimum size=4pt, label=left:$a$] (a3) at (5,4) {};
\node[shape=circle,draw=black,inner sep=0pt, minimum size=4pt, label=right:$b_3$](b3) at (7,4){};
\node[shape=circle,draw=black,inner sep=0pt, minimum size=4pt, label=left:$a_4$] (a4) at (5,2) {};
\node[shape=circle,draw=black,inner sep=0pt, minimum size=4pt, label=right:$b_4$](b4) at (7,2){};
...
\path[-,line width=1pt, decoration={markings, mark=at position .25 with {\arrow{latex}}}, blue] (b3) edge[decorate] (a3);
\path[-,line width=1pt, decoration={markings, mark=at position .2 with {\arrow{latex}}}, red] (a3) edge[decorate] (b4);
\path[-,line width=1pt, decoration={markings, mark=at position .25 with {\arrow{latex}}}, blue] (b4) edge[decorate] (a4);
\path[-,line width=1pt, decoration={markings, mark=at position .2 with {\arrow{latex}}}, red] (a4) edge[decorate] (b3);
...
\begin{proof}
  ...
  that the \DIFdelbegin \DIFdel{tight }\DIFdelend constraints are $x(\delta(v))\le 1$ \DIFaddbegin \DIFadd{are tight }\DIFaddend for $v\in V_x$
  and \eqref{eq:lin_descr_stab} \DIFaddbegin \DIFadd{are tight }\DIFaddend for $e\in E_x$, where
  $|V_x|+|E_x|=|E|$. Moreover, let us assume that we choose the $|E|$
  tight constraints so that $|V_x|$ is as large as possible.

The constraints $x(\delta(v))=1$, $v\in V$ are linearly dependent, \DIFdelbegin \DIFdel{in particular $\sum_{a\in\mathcal{A}}\chi(\delta(a))=\sum_{b\in\mathcal{B}}\chi(\delta(b))$}\DIFdelend \DIFaddbegin \DIFadd{since $\sum_{a\in\mathcal{A}}x(\delta(a))=\sum_{b\in\mathcal{B}}x(\delta(b))$ for every $x \in \R^E$}\DIFaddend . Hence, we have $V_x\subsetneq V$. On the other hand if $a=N_{\max}(b)$ \DIFaddbegin \DIFadd{for some $b \in V$ }\DIFaddend then $e:=ab\not\in E_x$. Indeed, $a=N_{\max}(b)$ implies $\delta^{>a}(b)=\varnothing$\DIFaddbegin \DIFadd{, }\DIFaddend then
$$
	1\le x(\delta^{>a}(b))+ x(\delta^{>b}(a)) + x_{ab}=x(\delta^{\ge b}(a)) \le x(\delta(a)) \le 1\,,
$$
showing that $\delta^{< b}(a)=\varnothing$\DIFdelbegin \DIFdel{and hence $E_x\setminus e$}\DIFdelend \DIFaddbegin \DIFadd{, and hence $E_x\setminus \{e\}$}\DIFaddend , $V_x\cup \{a\}$ also define the vertex $x$. \DIFaddbegin \DIFadd{Since $x$ has $|E|$ nonzero coordinates $|E_x \setminus \{e\}| + |V_x \cup \{a\}| \geq |E|$. Moreover, if $e \in E_x$ then $|E_x \setminus \{e\}| + |V_x \cup \{a\}| = |E|$ contradicting the choice of $V_x$, $E_x$. }\DIFaddend Analogously, we can show that if $b\in V_x$  and $a=N_{\min}(b)$ then $e:=ab\not\in E_x$. Moreover, notice that $N_{\min}(v)\neq N_{\max}(v)$ for $v\in V_x$ since no coordinate of $x$ equals~$1$. Thus, 
\[
...
$V(G')=V$ and $E(G') = E \setminus \{e\}$. \DIFaddbegin \DIFadd{Then 
}$$\DIFadd{P' = P(G') \cap \{x\in \R^{E(G')}: x(\delta^{>a}(b))+ x(\delta^{>b}(a))\geq1\}.}$$
\DIFaddend Define $H'$ to be the hyperplane $\{x\in \R^{E(G')}: x(\delta^{>a}(b))+ x(\delta^{>b}(a))=1\}$.  Then every vertex
of $P'$ is either a vertex of $P(G')$ or the intersection of an edge
of $P(G')$ with the hyperplane $H'$. \DIFdelbegin \DIFdel{Since vertices of $P(G')$ are
integral by }\DIFdelend \DIFaddbegin \DIFadd{By }\DIFaddend the minimal choice of graph $G$
\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin $$\DIFadd{P(G') = Q(G'),}$$ 
\DIFadd{that is to say vertices of $P(G')$ are
integral. So }\DIFaddend it remains to consider
...
straightforward to see that $x'$ is a vertex of \DIFdelbegin \DIFdel{$P(G')$}\DIFdelend \DIFaddbegin \DIFadd{$P(G')=Q(G')$}\DIFaddend . Thus by
