...
$\varnothing$ is the least preferred element of each vertex $u \in V$. In their seminal paper
...
The next Corollary \ref{cor:edge} can be obtained from Ratier's characterization of edges of the stable matching polytope~\cite{ratier1996stable}.
...
hyperplane
$\{x\in \R^{E(G')}: x(\delta^{>a}(b))+ x(\delta^{>b}(a))=1\}$.  Then every vertex
