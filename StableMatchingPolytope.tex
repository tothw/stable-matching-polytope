...
\begin{note}
Alternatively Corollary \ref{cor:edge} can be seen without Lemma \ref{lemma:edge} via Ratier's characterization of edges of the stable matching polytope \cite{ratier1996stable}. Since $J_1$ and $J_2$ are both non-empty, not all $a \in A$ agree on their preference between $M_1$ and $M_2$. That is to say, $M_1$ and $M_2$ are not comparable and by Ratier's theory do not share an edge in the stable matching polytope.
\end{note}
