...
over $v_2$ iff $v_1 >_u v_2$. For ease of notation, we will think of $>_v$ as a total
ordering on $N(v) \cup \{\varnothing\}$ where 
$\varnothing$ is the least preferred element of each vertex $v \in V$. In their seminal paper
...
\begin{note}Lemma \ref{lemma:sym_stable} is equivalent to an observation made by Conway on the way to the well-known lattice structure of stable matchings (see Conway's result in \cite{roth1992two}). In the interest of self-containment we provide a short elementary proof below.
\end{note}
\begin{proof}
...
\begin{note}
Alternatively Corollary \ref{cor:edge} can be seen without Lemma \ref{lemma:edge} via Ratier's characterization of edges of the stable matching polytope \cite{ratier1996stable}. Since $J_1$ and $J_2$ are both non-empty, not all $a \in \mathcal{A}$ agree on their preference between $M_1$ and $M_2$. That is to say, $M_1$ and $M_2$ are not comparable and by \cite{ratier1996stable} do not share an edge in the stable matching polytope.
\end{note}
